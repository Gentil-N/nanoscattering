...
def extract():
    ...
    for v in SELECTION:
        ...
        ax.set_title("(" + str(v[0]) + ", " + str(v[1]) + ")")

        x, y = ASC_DATA.extract_curve(v[0], v[1], ASC_BCK, ASC_REF)
        x *= 1e-9
        
        def save_spectrum(event):
            global HMIN
            global HMAX
        ...
        rs = SpanSelector(ax, on_select, 'horizontal', props=dict(facecolor='yellow', alpha=0.1), interactive=True)
        axeselect = fig.add_axes([0.8, 0.0, 0.2, 0.075])
        bselect = Button(axeselect, 'Save Spectrum')
        bselect.on_clicked(save_spectrum)
